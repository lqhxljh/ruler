...
//        mScroller = new ScaleScroller(getContext(), this);
    ...
    /**
     * @param color 设置报警时间的颜色
     */
    public void setAlarmBg(int color) {
        if (adapter != null) {
            adapter.setAlarmBg(color);
        }
    }
     ...
     * @param color 设置报警时间的颜色
     */
    public void setVideoBg(int color) {
        if (adapter != null) {
            adapter.setVideoBg(color);
        }
    }

    public void setVisitorBg(int color) {
        if (adapter != null) {
            adapter.setVisitorBg(color);
        }
    }
    ...
    private class MyLinearLayoutManager extends LinearLayoutManager {
        ...
        public void isCanScrollHorizontally(boolean isCanScrollHorizontally) {
    ...
    public void setIsCanScrollBar(boolean isCanScrollBar) {
        if (manager != null) {
            manager.isCanScrollHorizontally(isCanScrollBar);
        }
    }
...
//        ELog.e("mScreenWidth = " + mScreenWidth);
        ...
        addOnScrollListener(new OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                //获取左屏幕的偏移量
                int leftScrollXCalculated = (int) (Math.abs(firstVisibleItem.getLeft()) + firstVisibleItemPosition * (320 + zoom));
...
//        postInvalidate();//重绘
        adapter.setAlarmTimeSlot(videoTimeSlot);
    }
     ...
     */
    public synchronized void setCurrentPlayTimeMillis(long currentTimeMillis) {
...
//                        ELog.e("当前时间：" + DateUtils.getDateTime(currentTimeMillis));
    ...
    private void initPaint() {
        ...
        vedioAreaPaint.setColor(videoBg);
    }
...
//    }
    ...
    private int viewHeight = CUtils.dip2px(60);
...
//    }
     ...
     * 缩放中
     */
    public void setZoom(int scaleZoom) {
        if (zoom < -300)
            zoom = -300;
        zoom = scaleZoom;
        if (zoom < -320 / 2) {
